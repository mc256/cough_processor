...
class CoughDataSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        with open("%s/dnn2016_%d.pkl" % (DATA_PATH, row[0]), 'rb') as file_handler:
            ...
            # return torch.from_numpy( data['zxx_log'][:, row[1]: row[1] + 16]).to(DEVICE).double(), self.label.narrow(0, index, 1).max()
            return data['zxx_log'][:, row[1]: row[1] + 16], self.label[index: index + 1].item()
