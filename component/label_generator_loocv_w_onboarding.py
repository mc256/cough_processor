...
from component.configuration import ONBOARDING_BASE_PATH, ONBOARDING_DATA_PATH, SR, SEED, WINDOW_SIZE
from component.util import rms_to_db


def read_coarse_labels():
    # Read Current Labels
    table_fine = pd.read_csv(os.path.join(ONBOARDING_BASE_PATH, './fine_grained_annotation.csv'), index_col=0)
    ...
    table_coarse = pd.read_csv(os.path.join(ONBOARDING_BASE_PATH, 'coarse_grained_annotation.csv'), index_col=0)
...
def label_segmentation(label_list, decibel_lower_bound=0.0):
    ...
    for index, row in tqdm(label_list.iterrows(), total=label_list.shape[0]):
        ...
        audio_id = index
        filepath = os.path.join(ONBOARDING_DATA_PATH, "dnn2016_%d.pkl" % audio_id)
        try:
...
def get_labels_by_ids(exp, train_id, test_id, run=0):
    all_ids = read_coarse_labels()
    label_train = all_ids.loc[all_ids.index.isin(train_id)]
    label_val = all_ids.loc[all_ids.index.isin(test_id)]
...
def get_runs():
    with open('./label/loocv_split__onboarding.json', 'r') as json_handler:
        split = json.load(json_handler)
        list = []
        for item in split:
            list.append((item['test'], item['train']))
        return list
