...
from component.dataloader_multiprocessing_eval_w_onboarding import CoughDataSet
from component.decision_maker_w_onboarding import prepare_eval_data, evaluate_result
from component.label_generator_w_onboarding import get_labels
...
if __name__ == '__main__':
    ...
    for i in [1, 2, 3, 4]:
        # Get Labels
        segmented_label_train, label_val = get_labels(EXPERIMENT_NAME, i, train_size=TRAIN_SIZE, test_size=TEST_SIZE)
        ...
        torch.save(black_box.state_dict(), "./model/cnn_final_%s_run_%d.pkl" % (EXPERIMENT_NAME, i))
        black_box.eval()
        ...
        pd.DataFrame({
            ...
            'pred': y_scores,
            'actual': y_true
        }).to_csv('./results/cross_validation/mccv_%s_RUN_%d.csv' % (EXPERIMENT_NAME, i))
        ...
        try:
            ...
            fig.tight_layout()
            fig.savefig('./figures/prc_eval-%s-trials%d.png' % (EXPERIMENT_NAME, i))
            fig.show()
